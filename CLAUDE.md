...
- **`src/modules.py`** — `LowRank`: `W_u(W_v(x))` with input cast to the factor dtype and output cast back, which is what makes mixed `--model_dtype` / `--compressed_dtype` runs work. `HeadBlockLowRank` is the same contract for a head-partitioned projection under `--head_block_svd`: one shared `W_v` linear and a per-head `W_u` parameter, so the reconstruction is block diagonal in head space. Both expose `dense_weight()` and `factor_dtype`, which is what the `check_*` diagnostics use instead of reaching for `W_u.weight`.
...
| Per-matrix ceilings | `build_cap_map`, `matrix_ratio_cap`, `group_caps_tensor` |
...
- Every policy clamps against a **per-matrix** ceiling, not the scalar `--max_ratio`: the shell builds a `cap_map` once and hands it down beside `max_ratio`, and `waterfill_ratios` / `clamp_group_budget` / `bounded_proportional_split` take a per-entry tensor. `--min_rank_fraction 0.0` makes every entry `max_ratio`, which is exactly the old behaviour, and `0.05` reproduces `--max_ratio 0.9` on a square matrix. A new policy has to build its ceilings with `group_caps_tensor(cap_map, keys, max_ratio)` rather than reading the scalar.
- `--head_block_svd` swaps the joint SVD of `k_proj` / `v_proj` for one truncation per KV head, sharing the same whitening factor. It changes only the *realization* of a ratio, never the allocation, so the parameter budget stays exact; `head_block_rank` solves the per-head rank from the ratio. Its `rank_map` entries are `head_block_entry` dicts rather than plain integers, which is what keeps every checkpoint written before it readable, and `apply_lowrank` branches on `is_head_block_entry`. It requires `--run_v2` and is rejected alongside `--sequential_update`, whose two update paths both address `W_u`/`W_v` as inner linears.
...
- Adding a compression-target flag means updating `generate_paths`, `matrix_shapes_from_config` (which has to name the same matrices, for offline analysis), `head_partition_from_config` if its output partitions into heads, `build_run_name`, `--group_patterns`, and `MATRIX_TOKEN_MAP` in `generate_tables.py`.
- A spectral score's units carry `min(out, in)`: `eff_rank` is bounded by it, `entropy` by its log, `truncation` by the whitened Frobenius norm. Every family of an MHA model shares that bound, so raw scores are comparable there by accident; under GQA `k_proj` and `v_proj` carry a spectrum `heads / kv_heads` times shorter and score low for a purely dimensional reason. The `*_rel` metrics divide it out. Under `softmax_temp`, which min-max normalizes within the group, they are bit-identical to the raw form wherever every matrix in a group shares a shape.
